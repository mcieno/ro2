...
timelimit=12       # 12 seconds for finding the first solution
heurtime=60        # 1 minute for optimizing the initial solution
totaltimelimit=$(($timelimit+$heurtime+60))

bmdir="benchmarks"
bmsig="$bmdir/bm_$(date +%F_%T)"

bmfile="$bmsig.1min.matheur.csv"
bmfile_png="$bmsig.1min.matheur.png"
