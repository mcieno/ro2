...
bmfile_nodes="$bmsig.loop.4th.nodes.csv"
bmfile_times="$bmsig.loop.4th.times.csv"

bmfile_nodes_png="$bmsig.loop.4th.nodes.png"
bmfile_times_png="$bmsig.loop.4th.times.png"

mkdir -p $bmdir || exit 1

echo "[*] Saving benchmark to $bmsig.loop.4th.[nodes|times].csv"
...
python2 ./perfprof.py        \
    ...
    $bmfile_nodes            \
    $bmfile_nodes_png        \
    -P "Nodes 4th, shift 100"    > /dev/null
