...
bmfile_nodes="$bmsig.loop.1th.nodes.csv"
bmfile_times="$bmsig.loop.1th.times.csv"

bmfile_nodes_png="$bmsig.loop.1th.nodes.png"
bmfile_times_png="$bmsig.loop.1th.times.png"

mkdir -p $bmdir || exit 1

echo "[*] Saving benchmark to $bmsig.loop.1th.[nodes|times].csv"
...
for file in "${testbed[@]}"; do
    for seed in "${seeds[@]}"; do
        ...
        for model in "${models[@]}"; do
            testresult=( $(timeout $timelimit ./bin/tsp $file --model=$model --seed $seed --timelimit $timelimit --nodelimit $nodelimit -j1 --noplot --quiet) )
...
python2 ./perfprof.py        \
    ...
    $bmfile_nodes            \
    $bmfile_nodes_png        \
    -P "Nodes 1th, shift 100"    > /dev/null
