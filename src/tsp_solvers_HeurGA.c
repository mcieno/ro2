...
//in *best_fit the fitness value of best individual is stored
size_t
compute_fitness(double *fitness, size_t **population, instance *problem, double *best_fit)
{
...
void
GeneticAlgorithm_solve(instance *problem)
{
    ...
    double curr_best_fit = 0;

    initialize_population(population, problem);

    size_t idx = compute_fitness(fitness_population, population, problem, &curr_best_fit);

    double best_fit = curr_best_fit;
    ...
    for ( size_t i= 0; elapsedtime + 1e-3 < conf.heurtime; i++ ){

        crossover(offspring, population, problem);

        mutation(offspring, problem);

        idx = compute_fitness(fitness_offspring, offspring, problem, &curr_best_fit);
...
GeneticAlgorithm_model ( instance *problem )
{
    ...
    GeneticAlgorithm_solve( problem );
