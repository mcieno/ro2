...
_add_constraints_LazyMTZ ( const instance *problem, CPXENVptr env, CPXLPptr lp )
{
    ...
    for ( size_t i = 0; i < problem->nnodes; ++i )
    {
        for ( size_t j = 0; j < problem->nnodes; ++j )
        {
            ...
            if ( CPXnewcols( env, lp, 1, &obj, &lb, &ub, &ctype, &cname ) ) {
                fprintf( stderr, CFATAL "_add_constraints_LazyMTZ: CPXnewcols [%s]\n", cname );
                exit( EXIT_FAILURE );
            }

            if ( CPXgetnumcols( env, lp ) - 1 != _LazyMTZ_xpos( i, j, problem ) ) {
                fprintf( stderr, CFATAL "_add_constraints_LazyMTZ: CPXgetnumcols [%s: x(%zu, %zu)]\n",
                    cname, i + 1, j + 1 );
                exit( EXIT_FAILURE );
            }
        }
    ...
    for ( size_t h = 0; h < problem->nnodes; ++h )
    {
        ...
        if ( CPXnewrows( env, lp, 1, &rhs, &sense, NULL, &cname ) ) {
            fprintf( stderr, CFATAL "_add_constraints_LazyMTZ: CPXnewrows [%s]\n", cname );
            exit( EXIT_FAILURE );
        }
        ...
        for ( size_t i = 0; i < problem->nnodes; ++i )
        {
            ...
            if ( CPXchgcoef( env, lp, lastrow, _LazyMTZ_xpos( i, h, problem ), 1.0 ) ) {
                fprintf( stderr, CFATAL "_add_constraints_LazyMTZ: CPXchgcoef [%s: x(%zu, %zu)]\n",
                    cname, i + 1, h + 1 );
                exit( EXIT_FAILURE );
            }
        }
    ...
    for ( size_t i = 1; i < problem->nnodes; ++i )
    {
        ...
        if ( CPXgetnumcols( env, lp ) - 1 != _LazyMTZ_upos( i, problem ) ) {
            fprintf( stderr, CFATAL "_add_constraints_LazyMTZ: CPXgetnumcols [%s]\n", cname );
            exit( EXIT_FAILURE );
        }
    }
    ...
    if ( rmatind == NULL || rmatval == NULL ) {
        fprintf( stderr, CFATAL "_add_constraints_LazyMTZ: Not enough memory\n" );
        exit( EXIT_FAILURE );
    }
    ...
    for ( size_t j = 1; j < problem->nnodes; ++j )
    {
        for ( size_t i = 1; i < problem->nnodes; ++i )
        {
            ...
            if ( CPXaddlazyconstraints( env, lp, 1, 3, &rhs, &sense, &rmatbeg, rmatind, rmatval, &cname ) ) {
                fprintf( stderr, CFATAL "_add_constraints_LazyMTZ: CPXaddlazyconstraints [%s]\n", cname );
                exit( EXIT_FAILURE );
            }
        }
    }
